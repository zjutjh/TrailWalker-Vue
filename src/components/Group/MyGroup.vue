    ...
    <v-card v-else class="text-center" raised rounded="xl">
      ...
      <v-card-actions v-if="!isEnd" style="display:block;">
        <div v-if="$store.state.currentGroup.captain_id===$store.state.currentUser.id" style="margin:0.5rem;">
          <v-btn style="margin:0.5rem;" @click="unsubmit" color="error" v-if="$store.state.currentGroup.is_submit===1">取消提交
            <v-icon right>mdi-cancel</v-icon>
          </v-btn>
          <v-btn style="margin:0.5rem;"  @click="submit" color="primary" v-else>
            队伍提交({{
              $store.state.routes.find((route) => {
                return route.id === $store.state.currentGroup.route_id
              }).remain
            }})
            <v-icon right>mdi-checkbox-marked-circle</v-icon>
          </v-btn>
          <v-btn @click="$router.push('/Group/Update')" style="margin:0.5rem;"
                 v-if="$store.state.currentGroup.is_submit!==1">修改
            <v-icon right>mdi-wrench</v-icon>
          </v-btn>
          <v-btn style="margin:0.5rem;" @click="breakConfirm=true" color="error" v-if="$store.state.currentGroup.is_submit!==1">解散
      ...
      <v-list-item>
        <v-list-item-content>
          <v-list-item-subtitle color="red">报名期间每天提供名额</v-list-item-subtitle>
      ...
      <v-bottom-sheet v-model="MemberSheet">
        <v-sheet class="bottom-sheet" v-if="selectedMember">
          <div class="text-center drop" style="padding:1rem;">
            ...
            <Avataaars :src="selectedMember.logo"/><br/>

            <h2 >姓名</h2> {{ selectedMember.name }}
            <h2 >手机</h2> {{ selectedMember.phone }}
            <h2 >邮箱</h2> {{ selectedMember.email }}
            <h2  v-if="selectedMember.qq">QQ </h2>{{ selectedMember.qq }}
            <h2  v-if="selectedMember.wx_id">微信</h2> {{ selectedMember.wx_id }}
